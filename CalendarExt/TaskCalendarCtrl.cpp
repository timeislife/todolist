...
BOOL TASKCALITEM::UpdateTask(const ITaskList12* pTasks, HTASKITEM hTask, int nEditAttrib, DWORD dwCalcDates)
{
...
void CTaskCalendarCtrl::UpdateTasks(const ITaskList* pTasks, IUI_UPDATETYPE nUpdate, int nEditAttribute)
{
	...
	case IUI_ALL:
		DeleteData();
		BuildData(pTasks12, pTasks->GetFirstTask(), TRUE);
		bChange = TRUE;
		break;
		
	case IUI_EDIT:
		bChange = UpdateTask(pTasks12, pTasks12->GetFirstTask(), nUpdate, nEditAttribute, TRUE);
		break;
		
	case IUI_DELETE:
		bChange = RemoveDeletedTasks(pTasks12);
		break;
...
BOOL CTaskCalendarCtrl::RemoveDeletedTasks(const ITaskList12* pTasks)
{
...
BOOL CTaskCalendarCtrl::UpdateTask(const ITaskList12* pTasks, HTASKITEM hTask, IUI_UPDATETYPE nUpdate, int nEditAttribute, BOOL bAndSiblings)
{
	if (hTask == NULL)
		return FALSE;
									...
									BOOL bSelected, BOOL bToday)
{
	...
	for (int nTask = nStart; nTask < nNumTasks; nTask++)
	{
		...
		if (pTCI->dwTaskID == m_dwSelectedTaskID)
		{
			...
			// lighten the color if not focused
		...
		if (nTaskHeight >= MIN_TASK_HEIGHT)
		{
			...
			if (nOffset != -1)
			{
				...
				if (m_fontAltText.GetSafeHandle())
					pDC->SelectObject(&m_fontAltText);
				...
				if (m_fontAltText.GetSafeHandle())
					pDC->SelectObject(&m_DefaultFont);
			}
...
int CTaskCalendarCtrl::GetCellTasks(const COleDateTime& dtCell, CTaskCalItemArray& aTasks, BOOL bOrdered) const
{
	...
	while (pos)
	{
		...
		if (pTCI->bDone && !HasOption(TCCO_DISPLAYDONE))
			continue;
