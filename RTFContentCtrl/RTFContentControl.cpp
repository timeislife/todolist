...
static UINT sCF_RTF				= ::RegisterClipboardFormat(CF_RTF);
static UINT sCF_RETEXTOBJ		= ::RegisterClipboardFormat(CF_RETEXTOBJ);
static UINT sCF_EmbeddedObject	= ::RegisterClipboardFormat(_T("Embedded Object"));
static UINT sCF_RichTextFormat	= ::RegisterClipboardFormat(_T("Rich Text Format"));
...
BOOL CRTFContentControl::Paste(BOOL bSimple)
{
	...
	case 0:
		...
		else // may need conversion
		{
			...
			if (!cb.HasFormat(sCF_RTF) && 
				!cb.HasFormat(sCF_RETEXTOBJ) && 
				!cb.HasFormat(sCF_RichTextFormat) && 
				!cb.HasFormat(sCF_EmbeddedObject) &&
				cb.GetText(sHtml, CF_HTML))
			{
#ifdef _UNICODE
				// convert to unicode for unpackaging
				Misc::EncodeAsUnicode(sHtml, CP_UTF8);
#endif
				cb.UnpackageContent(sHtml, CF_HTML);
				cb.Close();
				...
				if (!sHtml.IsEmpty())
				{
					...
					{
#ifdef _UNICODE
						// convert back to Ansi for clipboard
						...
						// overwrite with RTF
						CClipboard(*this).SetText(sRTF, sCF_RTF);
					}
				}
			}
...
void CRTFContentControl::OnEditCopyAsHtml() 
{
	...
	if (CClipboard().GetText(sRTF, sCF_RTF))
...
BOOL CRTFContentControl::CopyRtfToClipboardAsHtml(const CString& sRTF, BOOL bAppend)
{
	...
	{
		CClipboard::PackageContent(sHtml, CF_HTML);
		...
		if (bAppend)
		{
			...
			cb.Backup();
			cb.AddData(sHtml, CF_HTML);
			
			return cb.Restore();
		}
		else
		{
			CClipboard(*this).SetText(sHtml, CF_HTML);
		}
	}
