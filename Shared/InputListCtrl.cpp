...
void CInputListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if (lpDrawItemStruct->itemAction & (ODA_DRAWENTIRE | ODA_SELECT))
	{
		...
		while (bRes)
		{
			...
			{ 
				if (nCol > 0)
					pDC->FillSolidRect(rCell.left, rCell.top, 1, rCell.Height(), GetSysColor(COLOR_3DSHADOW));
				
				// if its the last column and we're not tight up against the client edge
				// then draw the end line
				if (!bRes && rCell.right != rClient.right)
					pDC->FillSolidRect(rCell.right, rCell.top, 1, rCell.Height(), GetSysColor(COLOR_3DSHADOW));
			}
		...
		{
			if (!m_bVertGrid)
				pDC->FillSolidRect(rClient.left - 1, rItem.bottom - 1, rClient.Width(), 1, GetSysColor(COLOR_3DSHADOW));
			else
				pDC->FillSolidRect(rClient.left - 1, rItem.bottom - 1, rItem.right, 1, GetSysColor(COLOR_3DSHADOW));
		}
