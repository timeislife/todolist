...
HTREEITEM CTreeCtrlHelper::FindWidestItem(BOOL bExpandedOnly) const
{
	int nWidth = 0;
	return FindWidestItem(NULL, nWidth, bExpandedOnly);
}

HTREEITEM CTreeCtrlHelper::FindWidestItem(int& nWidth, BOOL bExpandedOnly) const
{
	nWidth = 0;
	return FindWidestItem(NULL, nWidth, bExpandedOnly);
}

HTREEITEM CTreeCtrlHelper::FindWidestItem(HTREEITEM htiParent, int& nWidth, BOOL bExpandedOnly) const
{
	// if this task has no children then return itself
	HTREEITEM htiChild = m_tree.GetChildItem(htiParent);
	...
	while (htiChild)
	{
		...
		{
			int nTextWidth = GraphicsMisc::GetTextWidth(m_tree.GetItemText(htiChild), m_tree) + 6;
			int nWidthChild = max(nTextWidth, (rChild.left + nTextWidth));

			HTREEITEM htiChildWidest = FindWidestItem(htiChild, nWidthChild, bExpandedOnly);
