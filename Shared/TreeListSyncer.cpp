...
BOOL CTreeListSyncer::Resync(HWND hwnd, HWND hwndTo, BOOL bIncSelection, BOOL bClearTreeSel)
{
	if (!m_bResyncEnabled || m_bResyncing)
		return FALSE;
...
void CTreeListSyncer::SelectTreeItem(HWND hwnd, HTREEITEM hti, BOOL bClear)
{
	...
	if (bClear)
	{
		//TRACE(_T("CTreeListSyncer::SelectTreeItem(bClear = TRUE)\n"));
		...
		case TVN_SELCHANGED:
			{
				...
				Resync(OtherWnd(hwnd), hwnd, TRUE, bClearSel);
			}
			break;
