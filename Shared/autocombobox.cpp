...
CAutoComboBox::CAutoComboBox(DWORD dwFlags) : m_dwFlags(dwFlags), m_bClosingUp(FALSE), m_bEditChange(FALSE), m_nDeleteItem(LB_ERR)
{
}
...
{
	CDialogHelper::RefreshMaxDropWidth(*this, NULL, 0, GetExtraListboxWidth());
}

int CAutoComboBox::GetExtraListboxWidth() const
{
	// space for close button
	return (IsType(CBS_DROPDOWNLIST) ? 0 : SIZE_CLOSEBTN);
}
...
{
	return ((COwnerdrawComboBoxBase::GetStyle() & 0xf) == nComboType);
}
...
BOOL CAutoComboBox::DeleteLBItem(int nItem)
{
	...
	if ((nItem >= 0) && (nItem < GetCount()))
	{
		...
		{
			nCurSel = CB_ERR; // we've delete the last item
		}
