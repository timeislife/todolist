...
CUrlRichEditCtrl::CUrlRichEditCtrl() : m_nContextUrl(-1), m_lpDragObject(NULL)
{
	...
	// file protocol is a bit special
	...
	if (bEnable)
	{
		...
		if (fReally && (*lpcfFormat == COutlookHelper::CF_OUTLOOK))
		{
			...
			for (short nSel = 1; nSel <= nNumSel; nSel++)
			{
				...
				sLinks += COutlookHelper::FormatItemAsUrl(item, (OAFMT_NICE | OAFMT_BRACED));
...
CString CUrlRichEditCtrl::GetUrl(int nURL, BOOL bAsFile) const
{
	...
	if (nURL >= 0 && nURL < m_aUrls.GetSize())
	{
		...
		if (sUrl.Find('#') ==  -1)
		{
			...
			// expand env vars
			FileMisc::ExpandPathEnvironmentVariables(sUrl);
		}
...
void CUrlRichEditCtrl::Paste()
{
	...
	if (cb.GetText(sClipText) && FileMisc::IsPath(sClipText))
	{
	...
	else
	{
		...
		// see if there is an associated URL
		CString sClipURL;
		
		// Internet Explorer
		static UINT CF_MSURL  = ::RegisterClipboardFormat(_T("msSourceUrl"));

		if (cb.GetText(sClipURL, CF_MSURL))
		{
			CString sTemp;
			sTemp.Format(_T("%s ( %s )"), sClipText, sClipURL);

			sClipText = sTemp;
		}
		else
		{
			// Firefox can use Ansi encoding for name so we test for both
			static UINT CF_MZURLA = ::RegisterClipboardFormatA("text/html");
			static UINT CF_MZURLW = ::RegisterClipboardFormatW(_T("text/html"));

			if (!cb.GetText(sClipURL, CF_MZURLA) &&
				!cb.GetText(sClipURL, CF_MZURLW))
			{
				// Chrome uses CF_HTML encoded as multibyte
				if (cb.GetText(sClipURL, CF_HTML))
				{
#ifdef _UNICODE
					Misc::EncodeAsUnicode(sClipURL);
#endif
				}
			}

			// process the HTML returned
			if (!sClipURL.IsEmpty())
			{
				CStringArray aLinks, aLinkText;
				int nLink = ExtractLinks(sClipURL, aLinks, aLinkText);
				...
				while (nLink--)
				{
					...
					sClipText.Replace(aLinkText[nLink], sLinkAndText);
				}
			}
		}
		cb.Close();

		// do the paste else fallback on default processing
		if (!sClipText.IsEmpty())
		{
			ReplaceSel(sClipText, TRUE);
		}
		else 
		{
...
int CUrlRichEditCtrl::ExtractLinks(LPCTSTR szHtml, CStringArray& aLinks, CStringArray& aLinkText)
{
	CString sHtml(szHtml);
	const CString sLinkStart(_T("href=\"")), sLinkEnd(_T("\""));
	const CString sTextStart(_T(">")), sTextEnd(_T("</a>"));
	
	int nStart = sHtml.Find(sLinkStart), nEnd = -1;
	
	while (nStart != -1)
	{
		nStart += sLinkStart.GetLength();
		nEnd = sHtml.Find(sLinkEnd, nStart);
		
		if (nEnd == -1)
			break;

		// else
		CString sLink = sHtml.Mid(nStart, (nEnd - nStart));
		
		// get text
		nStart = sHtml.Find(sTextStart, nEnd + 1);

		if (nStart == -1)
			break;
		
		nStart += sTextStart.GetLength();
		nEnd = sHtml.Find(sTextEnd, nStart);
		
		if (nEnd == -1)
			break;
		
		CString sText = sHtml.Mid(nStart, (nEnd - nStart));
		...
		// next link
		nStart = sHtml.Find(sLinkStart, nEnd);
	}
