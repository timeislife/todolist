...
//{{AFX_MSG_MAP(CFilteredToDoCtrl)
	...
	ON_NOTIFY(TVN_ITEMEXPANDED, IDC_TASKTREE, OnTreeExpandItem)
...
void CFilteredToDoCtrl::RebuildList(const SEARCHPARAMS& filter)
{
	...
	if (pLVData->sort.IsSorting() || !m_sort.IsSorting())
	{
...
BOOL CFilteredToDoCtrl::ModNeedsRefilter(TDC_ATTRIBUTE nModType, FTC_VIEW nView, DWORD dwModTaskID) const 
{
	...
	if (m_bCustomFilter) // 'Find' filter
	{
		...
		// don't refilter on Time Spent if time tracking or on comments editing
		if (bNeedRefilter && 
			((nModType == TDCA_TIMESPENT && IsActivelyTimeTracking()) ||
			 (nModType == TDCA_COMMENTS)))
		{
			bNeedRefilter = FALSE;
		}
	}
