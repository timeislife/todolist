...
BOOL CTaskFile::SetTaskAttributes(HTASKITEM hTask, const TODOITEM* pTDI)
{
	...
	if (pTDI->dwTaskRefID == 0)
	{
		...
		if ((pTDI->dTimeSpent != 0.0) || (pTDI->nTimeSpentUnits != TDITU_HOURS))
...
bool CTaskFile::TaskHasAttribute(HTASKITEM hTask, LPCTSTR szAttrib, BOOL bOmitHidden) const
{
	...
	if (!pXIAttrib || !pXIAttrib->GetValueLen()) // not found
	{
		...
		{
			return TaskHasAttribute(hTask, TDL_TASKTEXTCOLOR, bOmitHidden);
		}
		else if (lstrcmp(szAttrib, TDL_TASKWEBCOLOR) == 0)
		{
			return TaskHasAttribute(hTask, TDL_TASKTEXTWEBCOLOR, bOmitHidden);
		}
		else if (lstrcmp(szAttrib, TDL_TASKPARENTID) == 0)
		{
			return TaskHasAttribute(hTask, TDL_TASKID, bOmitHidden);
		}
