...
bool CTaskListCsvImporter::Import(LPCTSTR szSrcFilePath, ITaskList* pDestTaskFile, BOOL bSilent, IPreferences* pPrefs, LPCTSTR szKey)
{
	...
	ITaskList13* pTasks = GetITLInterface<ITaskList13>(pDestTaskFile, IID_TASKLIST12);
	ASSERT(pTasks);
...
BOOL CTaskListCsvImporter::ImportTask(ITaskList13* pTasks, const CString& sLine) const
{
...
void CTaskListCsvImporter::AddCustomAttributes(ITaskList13* pTasks) const
{
	int nAttrib = m_aColumnMapping.GetSize();
...
void CTaskListCsvImporter::AddCustomAttributesToTask(ITaskList13* pTasks, HTASKITEM hTask, const CStringArray& aValues) const
{
...
void CTaskListCsvImporter::AddAttributeToTask(ITaskList13* pTasks, HTASKITEM hTask, TDC_ATTRIBUTE nAttrib, const CStringArray& aValues) const
{
	ASSERT(nAttrib < TDCA_CUSTOMATTRIB_FIRST);

	int nCol = AttributeIndex(nAttrib);
	
	if ((nCol == -1) || (nCol >= aValues.GetSize()))
		return;
