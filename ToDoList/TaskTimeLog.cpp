...
CString TASKTIMELOGITEM::FormatRow(int nRowVer, const CString& sDelim) const
{
	...
	case VER_LATEST:
		sItem.Format(sRowFormat, 
					...
					CTimeHelper::FormatISOTime(dtFrom), 
					CDateHelper::FormatDate(dtTo, DHFD_ISO), 
					CTimeHelper::FormatISOTime(dtTo),
					Misc::Format(dHours, 3),
					sComment,
...
BOOL CTDCTaskTimeLog::LogTime(const TASKTIMELOGITEM& li, BOOL bLogSeparately)
{
	...
	sLog += li.FormatRow(m_nVersion, m_sDelim);
...
int CTDCTaskTimeLogAnalysis::BuildLogItemArray()
{
	...
	while (nItem--)
	{
		...
		if (!m_mapIDtoLogItem.Lookup(li.dwTaskID, nLogItem))
			m_mapIDtoLogItem[li.dwTaskID] = nItem;
	}
...
const TASKTIMELOGITEM& CTDCTaskTimeLogAnalysis::GetLogItem(DWORD dwTaskID) const
{
	// look up a Log item for reference
	int nLogIndex = -1;
	
	VERIFY(m_mapIDtoLogItem.Lookup(dwTaskID, nLogIndex));
	ASSERT(nLogIndex != -1);
	...
	case TTLF_CSV:
		{
			...
			if (file.Open(szOutputFile, CFile::modeCreate | CFile::modeWrite | CFile::typeText))
			{
				...
				for (int nItem = 0; nItem < nNumItems; nItem++)
				{
					...
					const TASKTIMELOGITEM& li = GetLogItem(dwTaskID);
					
					// write to file
					file.WriteString(FormatCsvRow(dwTaskID, li.sTaskTitle, dTime, li.sPath));
				}
	...
	case TTLF_XML:
		{
			...
			while (pos)
			{
				...
				pXITask->SetItemValue(_T("ID"), (int)li.dwTaskID);
				pXITask->SetItemValue(_T("TITLE"), li.sTaskTitle);
				pXITask->SetItemValue(_T("HOURS"), li.dHours);
				pXITask->SetItemValue(_T("PERSON"), li.sPerson);
			}
			...
			if (file.Open(szOutputFile, CFile::modeCreate | CFile::modeWrite | CFile::typeText))
			{
				...
				for (int nPeriod = 0; nPeriod < nNumPeriods; nPeriod++)
				{
					...
					for (int nItem = 0; nItem < nNumItems; nItem++)
					{
						...
						// write to file
						file.WriteString(FormatCsvRow(dwTaskID, li.sTaskTitle, dTime, li.sPath, sPeriod));
					}
	...
	case TTLF_XML:
		{
			...
			for (int nPeriod = 0; nPeriod < nNumPeriods; nPeriod++)
			{
				...
				while (posID)
				{
					...
					pXITask->SetItemValue(_T("HOURS"), li.dHours);
					pXITask->SetItemValue(_T("PERSON"), li.sPerson);
					pXITask->SetItemValue(_T("PATH"), li.sPath);
				}
			}
...
{
	ASSERT((CDateHelper::GetTimeOnly(date1) == 0) && (CDateHelper::GetTimeOnly(date2) == 0));
