...
void CToDoCtrl::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_TASKTREE, m_tree);
...
//{{AFX_MSG_MAP(CToDoCtrl)
	...
	ON_WM_SHOWWINDOW()
	ON_WM_TIMER()
	ON_NOTIFY(NM_SETFOCUS, IDC_TASKTREE, OnTreeChangeFocus)
	ON_NOTIFY(NM_KILLFOCUS, IDC_TASKTREE, OnTreeChangeFocus)
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_TASKTREE, OnTreeCustomDraw)
	ON_NOTIFY(TVN_GETDISPINFO, IDC_TASKTREE, OnTreeGetDispInfo)
	ON_NOTIFY(TVN_KEYDOWN, IDC_TASKTREE, OnTreeKeyDown)
	ON_NOTIFY(TVN_BEGINLABELEDIT, IDC_TASKTREE, OnTreeBeginlabeledit)
	ON_NOTIFY(TVN_SELCHANGED, IDC_TASKTREE, OnTreeSelChanged)
	ON_NOTIFY(TVN_GETINFOTIP, IDC_TASKTREE, OnTreeGetInfoTip)
...
void CToDoCtrl::LoadTaskIcons()
{
	...
	// update tree
	if (IsColumnShowing(TDCC_ICON)) // this overrides
...
void CToDoCtrl::InitEditPrompts()
{
	...
	m_mgrPrompts.SetComboEditPrompt(m_cbCategory, IDS_TDC_EDITPROMPT_CATEGORY);
	m_mgrPrompts.SetComboEditPrompt(m_cbTags, IDS_TDC_EDITPROMPT_TAGS);
	...
	m_mgrPrompts.SetComboEditPrompt(IDC_DUETIME, *this, CTimeHelper::Format24HourTime(23, 59));
	m_mgrPrompts.SetComboEditPrompt(IDC_STARTTIME, *this, CTimeHelper::Format24HourTime(0, 0));
...
{
	if ((nWidth == -1 || nWidth >= MINCOLWIDTH) && nWidth != m_nMaxColWidth)
	{
...
BOOL CToDoCtrl::IsCtrlShowing(const CTRLITEM& ctrl) const
{
	...
	{
	case IDC_COLOUR:
		if (m_visColAttrib.GetShowEditsAndFilters() != TDLSA_ANY)
		{
...
void CToDoCtrl::DrawSplitter(CDC* pDC) 
{
		...
		IsCommentsVisible())
	{
		...
		pDC->FillSolidRect(rSplitter, m_theme.crAppBackDark);

		// draw drag marker
		CRect rMarker(rSplitter);

		if (rSplitter.Height() > rSplitter.Width())
		{
			// vertical
			rMarker.DeflateRect((SPLITSIZE / 2) - 1, (rMarker.Height() / 2) - 10);
		}
		else
		{
			// horizontal
			rMarker.DeflateRect((rMarker.Width() / 2) - 10, (SPLITSIZE / 2) - 1);
		}

		// use the splitter bkgnd luminance to decide whether
		// to draw the marker lighter or darker
		if (RGBX(m_theme.crAppBackDark).Luminance() > 128)
			pDC->FillSolidRect(rMarker, m_theme.crAppLinesDark);
		else
			pDC->FillSolidRect(rMarker, m_theme.crAppLinesLight);
...
int CToDoCtrl::GetAllocToNames(CStringArray& aNames, BOOL bIncDefault) const 
{ 
	if (m_cbAllocTo.GetItems(aNames) && !bIncDefault)
		Misc::RemoveItems(m_aDefAllocTo, aNames);
	
	return aNames.GetSize();
}

int CToDoCtrl::GetAllocByNames(CStringArray& aNames, BOOL bIncDefault) const 
{ 
	if (m_cbAllocBy.GetItems(aNames) && !bIncDefault)
		Misc::RemoveItems(m_aDefAllocBy, aNames);
	
	return aNames.GetSize();
}

int CToDoCtrl::GetCategoryNames(CStringArray& aNames, BOOL bIncDefault) const 
{ 
	if (m_cbCategory.GetItems(aNames) && !bIncDefault)
		Misc::RemoveItems(m_aDefCategory, aNames);
	
	return aNames.GetSize();
}

int CToDoCtrl::GetTagNames(CStringArray& aNames, BOOL bIncDefault) const 
{ 
	if (m_cbTags.GetItems(aNames) && !bIncDefault)
		Misc::RemoveItems(m_aDefTags, aNames);
	
	return aNames.GetSize();
}

int CToDoCtrl::GetStatusNames(CStringArray& aNames, BOOL bIncDefault) const 
{ 
	if (m_cbStatus.GetItems(aNames) && !bIncDefault)
		Misc::RemoveItems(m_aDefStatus, aNames);
	
	return aNames.GetSize();
}

int CToDoCtrl::GetVersionNames(CStringArray& aNames) const 
{ 
...
{
	SetDefaultComboNames(m_cbAllocBy, aNames, m_aDefAllocBy, bReadOnly, TRUE);
...
{
	SetDefaultComboNames(m_cbStatus, aNames, m_aDefStatus, bReadOnly, TRUE);
...
double CToDoCtrl::CalcSelectedTaskTimeEstimate(TCHAR nUnits) const
{
	...
	CHTIList selection;
	Selection().CopySelection(selection, TRUE);
...
// 	{
// 	case TDCD_START:
// 		if (SetSelectedTaskDate(TDCD_START, date, FALSE))
// 		{
...
BOOL CToDoCtrl::OffsetSelectedTaskDate(TDC_DATE nDate, int nAmount, TDC_OFFSET nUnits, BOOL bAndSubtasks)
{
	...
	CHTIList htiSel;
	Selection().CopySelection(htiSel, bAndSubtasks);
...
BOOL CToDoCtrl::SelectionHasDependents() const
{
	POSITION pos = Selection().GetFirstItemPos();
...
BOOL CToDoCtrl::SetSelectedTaskDone(const COleDateTime& date, BOOL bDateEdited)
{
	...
	CHTIList selection;
	Selection().CopySelection(selection, bDone);
	...
	while (pos)
	{
		...
		int nItemRes = SetTaskDone(hti, dtDone, nSetChildren, TRUE);
		...
		if (bRecurring)
		{
			...
			else // create copy of task below existing task
			{
...
LRESULT CToDoCtrl::OnRecreateRecurringTask(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	AddTasksToTree(task, htiParent, hti, TDCR_YES, TRUE, TDCA_NEWTASK);

	// reset new task(s) state to 'undone' including children
	hti = Selection().GetFirstItem();
	ASSERT(hti);

	SetTaskDone(hti, 0.0, TDC_SETUNDONE, TRUE);
	...
	if (m_data.GetTaskNextOccurrence(dwTaskID, dtNext, bDueDate))
	{
		...
		// mark as changed
		SetModified(TRUE, TDCA_NEWTASK, dwNewTaskID);
...
int CToDoCtrl::SetTaskDone(HTREEITEM hti, const COleDateTime& date, 
						   TDC_SETTASKDONE nSetChildren, BOOL bUpdateExistingDate)
{
	DWORD dwTaskID = GetTaskID(hti);
	int nRes = 0;
	...
	if (bDoneChange || bUpdateExistingDate)
	{
	...
	// resync the tree check states
	SetTaskAndReferenceCheckStates(dwTaskID, bDone);
	...
	if (nSetChildren != TDC_SETLEAVE)
	{
		...
		while (htiChild)
		{
			// the 'FALSE' we pass tells the function not
			// to change an existing completion date if one exists
			if (SetTaskDone(htiChild, date, nSetChildren, FALSE) == SET_CHANGE)
				nRes = SET_CHANGE;
...
BOOL CToDoCtrl::IsSorting() const
{
	return m_sort.IsSorting();
}

BOOL CToDoCtrl::IsSortingBy(TDC_COLUMN nSortBy) const
{
...
BOOL CToDoCtrl::DeleteSelectedTask(BOOL bWarnUser, BOOL bResetSel)
{
	...
	if (m_dwTimeTrackTaskID && Selection().HasItem(m_dwTimeTrackTaskID))
	...
	if (bResetSel)
	{
		...
		while (htiNextSel)
		{
			if (Selection().HasItem(htiNextSel) || 
				Selection().HasSelectedParent(htiNextSel) || 
	...
	// now we can delete with impunity
	{
		...
		RemoveOrphanTreeItemReferences();
	}
...
BOOL CToDoCtrl::RemoveOrphanTreeItemReferences(HTREEITEM hti)
{
	BOOL bRemoved = FALSE;
...
void CToDoCtrl::SetTaskAndReferenceCheckStates(DWORD dwTaskID, BOOL bChecked)
{
	// if the passed in task is a reference then we need to update
	// the 'real' task's check state and the rest of the references
	dwTaskID = m_data.GetTrueTaskID(dwTaskID);

	// item itself
	HTREEITEM hti = m_find.GetItem(dwTaskID);
	m_tree.TCH().SetItemChecked(hti, bChecked);

	// references
	if (m_data.TaskHasReferences(dwTaskID))
	{
		CDWordArray aRefIDs;
		int nRef = m_data.GetReferencesToTask(dwTaskID, aRefIDs);

		while (nRef--)
		{
			HTREEITEM htiRef = m_find.GetItem(aRefIDs[nRef]);

			if (htiRef)
				m_tree.TCH().SetItemChecked(htiRef, bChecked);
		}
	}
}
...
LRESULT CToDoCtrl::OnEditEnd(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (!sText.IsEmpty())
	{
		...
		BOOL bNewTask = (m_dwEditingID == m_dwLastAddedID);
		
		if ((nRes == SET_CHANGE) || bNewTask)
		{
			SetModified(TRUE, (bNewTask ? TDCA_NEWTASK : TDCA_TASKNAME), m_dwEditingID);
		}
	}
...
{ 
	if (sStatus != m_sCompletionStatus)
	{
		...
		if (GetSafeHwnd() && IsColumnShowing(TDCC_STATUS))
			m_tree.RecalcGutterColumn(TDCC_STATUS);
	}
}
...
void CToDoCtrl::UpdateVisibleColumns()
{
	...
	// if the Icon column visibility has changed then we need to
...
void CToDoCtrl::SaveGlobals(CTaskFile& file) const
{
	...
	if (GetCategoryNames(aItems, FALSE))
		file.SetCategoryNames(aItems);

	if (GetTagNames(aItems, FALSE))
		file.SetTagNames(aItems);

	if (GetStatusNames(aItems, FALSE))
		file.SetStatusNames(aItems);

	if (GetAllocToNames(aItems, FALSE))
		file.SetAllocToNames(aItems);

	if (GetAllocByNames(aItems, FALSE))
...
TDC_FILE CToDoCtrl::Load(const CString& sFilePath, CTaskFile& tasks/*out*/)
{
	...
	switch (nFileErr)
	{
	case XFL_CANCELLED:			return TDCO_CANCELLED;
	case XFL_MISSINGROOT:		return TDCO_NOTTASKLIST;
	case XFL_BADMSXML:			return TDCO_BADMSXML;
	case XFL_NOENCRYPTIONDLL:	return TDCO_NOENCRYPTIONDLL;
	case XFL_UNKNOWNENCRYPTION:	return TDCO_UNKNOWNENCRYPTION;
	}
...
BOOL CToDoCtrl::LoadTasks(const CTaskFile& file)
{
	...
	if (file.GetTaskCount())
	{
		...
		DWORD dwFirstVis = GetTaskID(m_tree.GetFirstVisibleItem());
...
// 			// draw insertion marker if we are the very last item
// 			if (m_tree.TCH().GetLas)
...
void CToDoCtrl::DrawCommentsText(CDC* pDC, const TODOITEM* pTDI, const TODOSTRUCTURE* pTDS, const CRect& rText, TDI_STATE nState)
{
	if (!pTDI->sComments.IsEmpty())
	{
		...
		{
			sComments.LoadString(IDS_TDC_SEECOMMENTS);
		}
		...
		pDC->DrawText(sComments, (LPRECT)(LPCRECT)rText, (DT_LEFT | DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX | GraphicsMisc::GetRTLDrawTextFlags(m_tree)));
								 ...
								 COLORREF& crText, COLORREF& crBack, BOOL bRef) const
{
	...
	if (bDone)
	{
		...
		else if (bRef && (m_crReference != CLR_NONE))
		{
	...
	else // all incomplete tasks
	{
		...
		if (bRef && (m_crReference != CLR_NONE))
		{
			crText = m_crReference;
		}
		else
...
BOOL CToDoCtrl::ModNeedsResort(TDC_ATTRIBUTE nModType, const TDSORT& sort) const
{
...
LRESULT CToDoCtrl::OnTreeDragOver(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (nRes != DD_DROPEFFECT_NONE)
	{
		...
		if (pDDI->bLeftDrag)
		{
			...
			if (m_data.IsTaskReference(dwOverTaskID) && SelectionHasNonReferences())
			{
	...
	if (nRes)
	{
		...
		if (pMsg->message == WM_DD_DRAGDROP)
		{
			...
			else if (bMove)
			{
				...
				if (m_data.MoveTasks(cache.aSelTaskIDs, dwDestParentID, dwDestPrevSiblingID))
				{
					// get selected tasks ordered, and without duplicate subtasks
					CHTIList selection;
					Selection().CopySelection(selection, TRUE, TRUE);
					...
					// make sure moved item is visible
...
BOOL CToDoCtrl::CanMoveSelectedTask(TDC_MOVETASK nDirection) const
{
	...
	if (!selection.GetCount() || !Selection().ItemsAreAllSiblings(selection))
		return FALSE;
...
BOOL CToDoCtrl::MoveSelection(TDC_MOVETASK nDirection)
{
	if (CanMoveSelectedTask(nDirection))
	{
		...
		// 3. copy tree selection without duplicate subtasks
		CHTIList selection;
		...
		if (m_data.MoveTasks(aTaskIDs, dwDestParentID, dwDestPrevSiblingID))
		{
			// scope the hold-redraw
			{
				HOLD_REDRAW(*this, m_tree);
				POSITION pos = selection.GetHeadPosition();
	...
	default:
		{
			...
			// copy the incoming task ID so we can detect
			// if it was in fact a reference task so we can
			// tell 'GetTaskTextColors'
			DWORD dwTrueID = dwTaskID;

			if (!m_data.GetTask(dwTrueID, pTDI, pTDS))
				return;

			GetTaskTextColors(pTDI, pTDS, pColors->crText, pColors->crBack, (dwTaskID != dwTrueID));
...
BOOL CToDoCtrl::DrawItemColumn(const TODOITEM* pTDI, const TODOSTRUCTURE* pTDS, const NCGDRAWITEM* pNCGDI, TDI_STATE nState, DWORD dwRefID)
{
	...
	case TDCC_RECURRENCE:
		{
			CString sRecurrence = pTDI->trRecurrence.GetRegularity(FALSE);
	...
	case TDCC_ID:
		{
			...
			if (dwRefID)
				sID.Format(_T("(%lu) %lu"), dwTaskID, dwRefID);
			else
				sID.Format(_T("%lu"), dwTaskID);
	...
	case TDCC_STATUS:
		{
			CString sStatus(pTDI->sStatus);

			// if a task is completed and has no status and the completion status
			// has been specified then draw the completion status
			if (sStatus.IsEmpty() && !m_sCompletionStatus.IsEmpty() && m_data.IsTaskDone(pTDI, pTDS, TDCCHECKALL))
				sStatus = m_sCompletionStatus;

			DrawGutterItemText(pNCGDI->pDC, sStatus, rItem, pNCGDI->nTextAlign);
		}
		break;
	...
	case TDCC_PATH:
		if (pTDS->GetParentTaskID())
		{
			...
			sPath.FormatDC(pNCGDI->pDC, rItem.Width(), ES_PATH);
	...
	case TDCC_SUBTASKDONE:
		{
			...
			if (m_data.GetSubtaskTotals(pTDI, pTDS, nSubtasksCount, nSubtasksDone))
			{
				...
				DrawGutterItemText(pNCGDI->pDC, sText, rItem, pNCGDI->nTextAlign);
			}
		}
		break;
	...
	case TDCCA_ICON:
		if (!data.IsEmpty())
		{
			...
			case DT_CENTER:
				...
				if (sName.IsEmpty())
				{
					pt.x = (rItem.left + ((rItem.Width() - nTotalWidth) / 2));
					break;
				}
				else 
				{
...
BOOL CToDoCtrl::WantDrawGutterTime(TDC_DATE nDate) const
{
	switch (nDate)
	{
	case TDCD_START:		
...
{	
	...
	if (CDateHelper::IsDateSet(date))
	{
		...
		if (WantDrawGutterTime(nDate) && (CDateHelper::GetTimeOnly(date) > 0))
		{
...
void CToDoCtrl::DrawGutterItemText(CDC* pDC, const CString& sText, const CRect& rect, int nAlign, COLORREF crText, CFont* pFont, BOOL bSymbol)
{
	...
	UINT nFlags = (nAlign | DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX | GraphicsMisc::GetRTLDrawTextFlags(m_tree));
	...
	if (bSymbol)
		GraphicsMisc::DrawSymbol(pDC, (char)sText[0], rect, nFlags, pFont);
	else
...
void CToDoCtrl::DrawGutterItemDate(CDC* pDC, const COleDateTime& date, TDC_DATE nDate, const CRect& rect, COLORREF crText)
{
	...
	if (WantDrawGutterTime(nDate))
	{
		...
		if (sTime.IsEmpty())
		{
			...
			COLORREF crActualText = ((crText != CLR_NONE) ? crText : pDC->GetTextColor());
		...
		if (!sTime.IsEmpty())
		{
			DrawGutterItemText(pDC, sTime, rDraw, TA_RIGHT, crTime);
...
BOOL CToDoCtrl::CopyCurrentSelection() const
{
	...
	VERIFY(Selection().CopySelection(selection, FALSE, TRUE));
	VERIFY(Selection().GetItemTitles(selection, aTitles));
							   ...
							   TDC_RESETIDS nResetIDs, BOOL bSelectAll, TDC_ATTRIBUTE nModAttrib)
{
	...
	while (hTask)
	{
		...
		if (bSelectAll)
			lstNewSel.AddTail(htiDestAfter);
	...
	// restore selection
	if (bSelectAll && !lstNewSel.IsEmpty())
	{
		Selection().SetItems(lstNewSel, TSHS_SELECT);
		m_tree.TCH().SelectItem(lstNewSel.GetHead());
	}
	else if (dwSelID)
	{
		SelectTask(dwSelID);
...
LRESULT CToDoCtrl::OnGutterNotifyItemClick(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (htiHit)
	{
		...
		case WM_LBUTTONDBLCLK:
			if (htiHit && (pNGIC->nColID == NCG_CLIENTCOLUMNID) && !IsReadOnly())
			{
...
void CToDoCtrl::EndTimeTracking(BOOL bAllowConfirm)
{
	...
	if (m_dwTimeTrackTaskID)
	{
		...
		if (HasStyle(TDCS_LOGTIMETRACKING))
		{
			...
			DoAddTimeToLogFile(m_dwTimeTrackTaskID, m_dLogTime, bShowDialog);
		}
...
BOOL CToDoCtrl::SelectTasks(const CDWordArray& aTaskIDs, BOOL bTrue)
{
	...
	if (nNumIDs > 0)
	{
		// Save current selection in case something goes wrong
		TDCSELECTIONCACHE cache;
		CacheTreeSelection(cache);
		...
		CHTIMap mapLookup;
		m_tree.TCH().BuildHTIMap(mapLookup);
		...
		for (int nID = 0; nID < nNumIDs; nID++)
		{
			...
			// select the first task 'properly' and then just add the rest
			HTREEITEM hti = NULL;

			if (!mapLookup.Lookup(dwTaskID, hti))
			{
				RestoreTreeSelection(cache);
				return FALSE;
			}
...
LRESULT CToDoCtrl::OnGutterRecalcColWidth(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	case TDCC_SUBTASKDONE:
		{
			...
			CString sLongest = m_find.GetLongestValue(nAttrib);
	...
	case TDCC_STATUS:
		{
			// determine the longest visible string
			CString sLongest = m_find.GetLongestValue(TDCA_STATUS, m_sCompletionStatus);
			pNCRC->nWidth = GraphicsMisc::GetAverageMaxStringWidth(sLongest, pNCRC->pDC);
		}
		break;
	...
	case TDCC_TAGS:
		{
			...
			if (GetColumnAttribAndCtrl(nColID, nAttrib, pCtrl))
			{
				...
				CString sLongest = m_find.GetLongestValue(nAttrib, sEdit);
	...
	case TDCC_STARTDATE:
	case TDCC_DONEDATE:
		pNCRC->nWidth = CalcMaxDateColWidth(TDC::MapColumnToDate(nColID), pNCRC->pDC);
		break;
			...
			else 
			{
				...
				case TDCCA_ICON:
					if (attribDef.IsList())
					{
						...
						case TDCCA_FIXEDMULTILIST:
							pNCRC->nWidth = (attribDef.aDefaultListData.GetSize() * 18);
							break;
						}
					}
	...
	// minimum width
	else if (pNCRC->nWidth < MINCOLWIDTH)
	{
		pNCRC->nWidth = MINCOLWIDTH;
	}
...
int CToDoCtrl::GetSelectedTasks(CTaskFile& tasks, const TDCGETTASKS& filter, DWORD dwFlags) const
{
	...
	// subtasks anyway
	CHTIList selection;
	Selection().CopySelection(selection, bWantSubtasks, TRUE);
...
void CToDoCtrl::CacheTreeSelection(TDCSELECTIONCACHE& cache) const
{
	...
	cache.dwFirstVisibleTaskID = GetTaskID(m_tree.GetFirstVisibleItem());
...
BOOL CToDoCtrl::RestoreTreeSelection(const TDCSELECTIONCACHE& cache)
{
	...
	if (CToDoCtrl::RestoreTreeSelection(cache.aSelTaskIDs, dwFocusedTaskID))
	{
		// restore scroll pos
		if (cache.dwFirstVisibleTaskID)
		{
			HTREEITEM htiFirstVis = m_find.GetItem(cache.dwFirstVisibleTaskID);
...
BOOL CToDoCtrl::RestoreTreeSelection(const CDWordArray& aTaskIDs, DWORD dwDefaultID)
{
	if (!MultiSelectItems(aTaskIDs)) // nothing selected
	{
...
int CToDoCtrl::GetSelectedTaskIDs(CDWordArray& aTaskIDs, DWORD& dwFocusedTaskID, BOOL bRemoveChildDupes) const
{
	...
	CHTIList selection;
	Selection().CopySelection(selection, bRemoveChildDupes);
								  ...
								  HTASKITEM hTask, const TDCGETTASKS& filter, BOOL bTitleCommentsOnly) const
{
	...
	GetTaskTextColors(pTDI, pTDS, crText, crBack, FALSE); // FALSE = 'not a reference'
	...
	if (pTarget == &m_tree)
	{
		...
		if (pData->hti && (bSingleFile || bSingleOObj))
		{
			...
			if (bSingleOObj)
			{
				...
				sFile = COutlookHelper::FormatItemAsUrl(item, OAFMT_NICE);
			}
...
int CToDoCtrl::CreateTasksFromOutlookObjects(TLDT_DATA* pData)
{
	...
	if (tasks.GetTaskCount())
	{
		...
		if (!htiInsert)
			htiInsert = m_tree.HitTest(CPoint(1, pData->ptClient.y));
...
void CToDoCtrl::SaveSortState(CPreferences& prefs) const
{
	...
	if (!sKey.IsEmpty())
	{
		prefs.WriteProfileInt(sKey, _T("Multi"), m_sort.bMulti);

		prefs.WriteProfileInt(sKey, _T("Column"), m_sort.single.nBy);
		prefs.WriteProfileInt(sKey, _T("Ascending"), m_sort.single.bAscending);

		prefs.WriteProfileInt(sKey, _T("Column1"), m_sort.multi.col1.nBy);
		prefs.WriteProfileInt(sKey, _T("Column2"), m_sort.multi.col2.nBy);
		prefs.WriteProfileInt(sKey, _T("Column3"), m_sort.multi.col3.nBy);
		prefs.WriteProfileInt(sKey, _T("Ascending1"), m_sort.multi.col1.bAscending);
		prefs.WriteProfileInt(sKey, _T("Ascending2"), m_sort.multi.col2.bAscending);
		prefs.WriteProfileInt(sKey, _T("Ascending3"), m_sort.multi.col3.bAscending);
...
void CToDoCtrl::LoadSortState(const CPreferences& prefs)
{
	CString sKey = GetPreferencesKey(_T("SortColState"));
	
	// single sort
	m_sort.single.nBy = (TDC_COLUMN)prefs.GetProfileInt(sKey, _T("Column"), TDCC_NONE);
	m_sort.single.bAscending = prefs.GetProfileInt(sKey, _T("Ascending"), TRUE);

	// multi sort
	m_sort.bMulti = prefs.GetProfileInt(sKey, _T("Multi"), FALSE);

	m_sort.multi.col1.nBy = (TDC_COLUMN)prefs.GetProfileInt(sKey, _T("Column1"), TDCC_NONE);
	m_sort.multi.col1.bAscending = prefs.GetProfileInt(sKey, _T("Ascending1"), TRUE);
	m_sort.multi.col2.nBy = (TDC_COLUMN)prefs.GetProfileInt(sKey, _T("Column2"), TDCC_NONE);
	m_sort.multi.col3.nBy = (TDC_COLUMN)prefs.GetProfileInt(sKey, _T("Column3"), TDCC_NONE);
	m_sort.multi.col2.bAscending = prefs.GetProfileInt(sKey, _T("Ascending2"), TRUE);
	m_sort.multi.col3.bAscending = prefs.GetProfileInt(sKey, _T("Ascending3"), TRUE);
...
void CToDoCtrl::SaveExpandedState(CPreferences& prefs) const
{
	ASSERT (GetSafeHwnd());
...
int CToDoCtrl::SaveTreeExpandedState(CPreferences& prefs, const CString& sKey, 
...
LRESULT CToDoCtrl::OnGutterGetCursor(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	default:
		...
		if (!IsReadOnly() && CTDCCustomAttributeHelper::IsCustomColumn((TDC_COLUMN)pNCGGC->nColID))
		{
			...
			case TDCCA_ICON:
				bShowHand = (!attribDef.IsList() || (attribDef.GetListType() == TDCCA_FIXEDLIST));
				break;
			}
		}
	}
...
BOOL CToDoCtrl::CheckIn()
{
	...
	if (m_sLastSavePath.IsEmpty()) // not yet saved
	{
		m_bCheckedOut = FALSE;
		return TRUE;
	}
...
void CToDoCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	if (m_bSplitting)
	{
		...
		if (nNewSize != nCommentsSize)
		{
			...
			if (CalcRequiredControlsRect(rClient, rCtrls, nCols, nRows, TRUE))
			{
				CRect rIntersect;

				if (rIntersect.IntersectRect(rClient, rCtrls) && (rIntersect == rCtrls))
				{
					Resize(0, 0, TRUE);
					Invalidate(TRUE);
					UpdateWindow();
...
CRect CToDoCtrl::GetSplitterRect() const
{
	...
	CRect rTasks = GetCtrlRect(IDC_TASKTREE);
...
void CToDoCtrl::ExpandTasks(TDC_EXPANDCOLLAPSE nWhat, BOOL bExpand)
{
	...
	if (!bExpand)
	{
		HTREEITEM htiSel = Selection().GetFirstItem(), hti = htiSel;

		while (!m_tree.TCH().IsItemVisible(hti))
		{
...
BOOL CToDoCtrl::UndoLastAction(BOOL bUndo)
{
	...
	if (m_data.CanUndoLastAction(bUndo))
	{
		...
		if (m_data.UndoLastAction(bUndo, aElms))
		{
			...
			// restore selection
			if (!aTaskIDs.GetSize() || !m_selection.SelectTasks(aTaskIDs))
			{
...
void CToDoCtrl::Sort(TDC_COLUMN nBy, BOOL bAllowToggle)
{
	...
	if (nBy != TDCC_NONE)
	{
		...
		if ((bAscending == -1) || !m_sort.single.IsSortingBy(nBy))
		{
			...
			{
				bAscending = (m_tree.GetGutterColumnSort(nBy) != NCGSORT_DOWN);
			}
		}
		...
		m_tree.SetGutterColumnSort(nBy, m_sort.single.bAscending ? NCGSORT_UP : NCGSORT_DOWN);
	}
	else
	{
		m_tree.SetGutterColumnSort(0, NCGSORT_NONE);
		m_sort.single.bAscending = TRUE;
	}
...
void CToDoCtrl::SortTree(HTREEITEM htiRoot, BOOL bSortChildren)
{
	TDSORTPARAMS ss(*this, m_sort);
...
void CToDoCtrl::SortTreeItem(HTREEITEM hti, const TDSORTPARAMS& ss)
{
	...
	BOOL bMultiSort = ss.sort.bMulti;

	TVSORTCB tvs = { hti, (bMultiSort ? CToDoCtrl::SortFuncMulti : CToDoCtrl::SortFunc), (LPARAM)&ss };
...
int CToDoCtrl::SortTasks(LPARAM lParam1, LPARAM lParam2, 
						 ...
						 BOOL bSortDueTodayHigh, DWORD dwTimeTrackedID)
{
	...
	{
		BOOL bHasReminder1 = tdc.TaskHasReminder(dwTaskID1);
		BOOL bHasReminder2 = tdc.TaskHasReminder(dwTaskID2);
	...
	else if (sort.IsSortingByCustom())
	{
		...
		// this can still fail
		if (!CTDCCustomAttributeHelper::GetAttributeDef(sort.nBy, tdc.m_aCustomAttribDefs, attribDef))
			return 0;

		return tdc.m_data.CompareTasks(dwTaskID1, dwTaskID2, attribDef, sort.bAscending);
	}

	// else default attribute
	return tdc.m_data.CompareTasks(dwTaskID1, dwTaskID2, sort.nBy, sort.bAscending, bSortDueTodayHigh);
}
