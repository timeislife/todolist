...
TODOITEM* CToDoCtrlData::NewTask(const TODOITEM& tdiRef, DWORD dwParentTaskID, const CTDCAttributeArray* pParentAttribs) const
{
...
int CToDoCtrlData::GetTaskLocalDependencies(DWORD dwTaskID, CDWordArray& aDependencies) const
{
	...
	return pTDI->GetLocalDependencies(aDependencies);
}
...
void CToDoCtrlData::ResetCachedCalculations(DWORD dwTaskID, const TODOITEM* pTDI, TDC_ATTRIBUTE nAttrib, BOOL bAndParents, BOOL bAndSubtasks) const
{
	...
	if (!pTDI)
	{
		pTDI = GetTask(dwTaskID, FALSE); // FALSE => returns references
		ASSERT(pTDI);
...
void CToDoCtrlData::ResetCachedCalculations(DWORD dwTaskID, const TODOITEM* pTDI, TDC_ATTRIBUTE nAttrib) const
{
	...
	case TDCA_POSITION:
		// reset task itself, parents and subtasks and fall thru
		ResetCachedCalculations(dwTaskID, pTDI, nAttrib, TRUE, TRUE);
	...
	case TDCA_DELETE:
		{
			...
			// also affects parent chain in a big way
			TODOSTRUCTURE* pTDSParent = NULL;
			int nPos = 0;

			if (LocateTask(dwTaskID, pTDSParent, nPos) && pTDSParent)
			{
				// reset parent once only
				const TODOITEM* pTDIParent = GetTask(pTDSParent);
				ResetCachedCalculations(pTDSParent->GetTaskID(), pTDIParent, TDCA_ALL, TRUE, FALSE);

				// then siblings after 'us' excluding parent
...
int CToDoCtrlData::SetTaskDate(DWORD dwTaskID, TDC_DATE nDate, const COleDateTime& date)
{
	...
	if (dtCur != dtDate)
	{
		...
		FixupTaskLocalDependencyDates(dwTaskID, nDate);
...
int CToDoCtrlData::OffsetTaskDate(DWORD dwTaskID, TDC_DATE nDate, int nAmount, TDC_OFFSET nUnits, BOOL bAndSubtasks, BOOL bForceWeekday)
{
	// convert units
	DH_UNITS nDHUnits;
	...
	int nRes = SetTaskDate(dwTaskID, nDate, date);
...
int CToDoCtrlData::InitMissingTaskDate(DWORD dwTaskID, TDC_DATE nDate, const COleDateTime& date, BOOL bAndSubtasks)
{
...
int CToDoCtrlData::SetTaskDependencies(DWORD dwTaskID, const CStringArray& aDepends)
{
	...
	int nRes = EditTaskAttribute(dwTaskID, pTDI, TDCA_DEPENDENCY, pTDI->aDependencies, aWeeded);
	...
	if (nRes == SET_CHANGE)
	{
		...
		if (HasStyle(TDCS_AUTOADJUSTDEPENDENCYDATES))
		{
			...
			while (nDepends--)
			{
				DWORD dwIDDepends = aDependIDs[nDepends];
				ASSERT(dwIDDepends);

				// the user may be still typing, so we just ignore
				// 'unknown' tasks
				if (HasTask(dwIDDepends))
				{
					...
					if (pTDIDepends && (pTDIDepends->IsDone() || pTDIDepends->HasDue()))
					{
						...
						UINT nAdjusted = AdjustTaskLocalDependencyDates(dwTaskID, dwIDDepends, nDate);
						
						// and then fix up our dependents
						if (Misc::HasFlag(nAdjusted, ADJUSTED_DUE))
							FixupTaskLocalDependencyDates(dwTaskID, TDCD_DUE);
					}
...
double CToDoCtrlData::CalcTimeEstimate(const TODOITEM* pTDI, const TODOSTRUCTURE* pTDS, 
										int nUnits, BOOL bReturnWeighted) const
{
	ASSERT (pTDS && pTDI);
	...
	if (pTDI->AttribNeedsRecalc(TDIR_TIMEESTIMATE))
	{
		...
		if (bIsParent) // children's time
		{
			for (int nSubTask = 0; nSubTask < pTDS->GetSubTaskCount(); nSubTask++)
			{
				...
				dEstimate += CalcTimeEstimate(pTDIChild, pTDSChild, THU_HOURS, TRUE); // TRUE == unadjusted by %
	...
	double dReturn = (bReturnWeighted ? dWeightedEstimate : dEstimate);
		...
		case TDCA_PATH:
			{
				...
				if ((sp.GetOperator() == FO_EQUALS) || (sp.GetOperator() == FO_NOT_EQUALS))
				{
					if (FileMisc::IsPathTerminated(sp.ValueAsString()))
						FileMisc::TerminatePath(sPath);
					else
						FileMisc::UnterminatePath(sPath);
				}
...
{
	double dDate = date.m_dt, dSearch = sp.ValueAsDate().m_dt;
	BOOL bMatch = FALSE;
...
BOOL CToDoCtrlData::TaskMatches(const CStringArray& aItems, const SEARCHPARAM& sp, SEARCHRESULT& result) const
{
	...
	Misc::Split(sp.ValueAsString(), aSearchItems, _T(""), TRUE);
...
UINT CToDoCtrlData::AdjustTaskLocalDependencyDates(DWORD dwTaskID, DWORD dwDependencyID, TDC_DATE nDate)
{
	...
	if (pTDI && !pTDI->IsDone())
	{
		...
		if (pTDIDepends)
		{
			...
			{
				dtNewStart = CDateHelper::ToWeekday(dtNewStart);

				// bump the due date too if present but before
				// we set the start date
				if (pTDI->HasDue() && pTDI->HasStart())
				{
					double dLength = CalcDuration(pTDI->dateStart, pTDI->dateDue, TRUE);

					COleDateTime dtNewDue = AddDuration(dtNewStart, dLength, TRUE);
...
void CToDoCtrlData::FixupTaskLocalDependencyDates(DWORD dwTaskID, TDC_DATE nDate)
{
	if (!HasStyle(TDCS_AUTOADJUSTDEPENDENCYDATES))
		return;
	...
	while (nDepend--)
	{
		...
		UINT nAdjusted = AdjustTaskLocalDependencyDates(dwIDDependent, dwTaskID, nDate);
		
		// then process this task's dependents
		// if the due date was actually modified
		if (Misc::HasFlag(nAdjusted, ADJUSTED_DUE))
		{
			FixupTaskLocalDependencyDates(dwIDDependent, TDCD_DUE);
		}
	}
}
