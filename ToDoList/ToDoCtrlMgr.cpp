...
BOOL CToDoCtrlMgr::CheckOut(int nIndex)
{
	CString sTemp;
	return CheckOut(nIndex, sTemp);
}

BOOL CToDoCtrlMgr::CheckOut(int nIndex, CString& sCheckedOutTo)
{
	ASSERTVALIDINDEX(nIndex);
...
BOOL CToDoCtrlMgr::CheckIn(int nIndex)
{
	ASSERTVALIDINDEX(nIndex);
	CFilteredToDoCtrl& tdc = GetToDoCtrl(nIndex);
...
int CToDoCtrlMgr::RemoveToDoCtrl(int nIndex, BOOL bDelete)
{
	...
	if (bDelete) // else it's going to be re-added
	{
		...
		if (m_mapBrowsers.Lookup(&tdc, pBrowser))
		{
			...
			FileMisc::DeleteFile(FileMisc::GetTempFileName(sTempFile, _T("html")), TRUE);
			FileMisc::DeleteFile(FileMisc::GetTempFileName(sTempFile, _T("txt")), TRUE);
		}
...
BOOL CToDoCtrlMgr::AddToSourceControl(int nIndex, BOOL bAdd)
{
	...
	if (!bAdd && !IsCheckedOut(nIndex) && !CheckOut(nIndex))
		return FALSE;
...
int CToDoCtrlMgr::UpdateTabItemImage(int nIndex) const
{
	...
	else if (tci.pTDC->IsActivelyTimeTracking())
	{
...
BOOL CToDoCtrlMgr::CreateBackup(const CString& sPath, const CString& sBackupFolder, int nKeepBackups)
{
	if (nKeepBackups)
	{
		...
		CStringArray aFiles;
		int nFiles = FileMisc::FindFiles(sFolder, aFiles, FALSE, sFName + _T("*") + sExt);
