...
CToDoListWnd::~CToDoListWnd()
{
	...
	FileMisc::DeleteFile(FileMisc::GetTempFileName(_T("ToDoList.print"), _T("html")), TRUE);
	FileMisc::DeleteFile(FileMisc::GetTempFileName(_T("ToDoList.import"), _T("txt")), TRUE);
	FileMisc::DeleteFile(FileMisc::GetTempFileName(_T("tdt")), TRUE);
}
...
void CToDoListWnd::InitMenuIconManager()
{
	...
	if (m_theme.HasToolbarImageFile(_T("TODOLIST_EXTRA")))
	{
		...
		m_mgrMenuIcons.AddImages(aCmdIDs, nExtraImageID, RGB(255, 0, 255));
}
...
TDC_FILE CToDoListWnd::SaveTaskList(int nIndex, LPCTSTR szFilePath, BOOL bAuto)
{
	...
	if (bUsesStorage)
	{
		...
		storageInfo.SetLocalFilePath(FileMisc::GetTempFileName());

		// save to file and then to storage
		if (tdc.Save(tasks, storageInfo.szLocalFileName) == TDCO_SUCCESS)
		{
			...
			while (TRUE)
			{
				if (sFilePath.IsEmpty()) // means first time save
				{
					...
					if (CDriveInfo::IsReadonlyPath(sFilePath) > 0)
					{
						...
						if (MessageBox(sMessage, IDS_SAVETASKLIST_TITLE, MB_OKCANCEL) == IDCANCEL)
						{
							return TDCO_CANCELLED; // user elected not to proceed
						}
						else
				...
				if (nResult != TDCO_SUCCESS)
				{
					...
					if (!bAuto)
					{
						...
						case TDCO_NOTALLOWED:
							{
								sMessage.Format(IDS_SAVEACCESSDENIED, sFilePath);
								
								if (IDYES == MessageBox(sMessage, IDS_SAVETASKLIST_TITLE, MB_YESNOCANCEL | MB_ICONEXCLAMATION))
								{
									sFilePath.Empty(); // try again
									continue;
								}
...
BOOL CToDoListWnd::OnEraseBkgnd(CDC* pDC) 
{
	...
	pDC->FillSolidRect(rClient, (CThemed::IsThemeActive() ? m_theme.crAppBackLight : GetSysColor(COLOR_3DFACE)));
...
TDC_FILE CToDoListWnd::OpenTaskList(CFilteredToDoCtrl* pTDC, LPCTSTR szFilePath, TSM_TASKLISTINFO* pInfo)
{
	...
	{
		ASSERT(FileMisc::IsTempFile(sFilePath));
...
BOOL CToDoListWnd::DoDueTaskNotification(int nTDC, int nDueBy)
{
	...
	pDDNotify->sExportPath = FileMisc::GetTempFileName(pDDNotify->sExportPath, (bHtmlNotify ? _T("html") : _T("txt")));
...
{
	static CString sTitle(_T("ToDoList 6.9.1"));
...
{
	// Don't localise version or contributions
	CString sVersion;
	sVersion.Format(_T("<b>%s</b> (c) AbstractSpoon 2003-14"), GetTitle());
	CLocalizer::IgnoreString(sVersion);

	CString sContribution;
	sContribution.LoadString(IDS_ABOUTCONTRIBUTION);
	CLocalizer::IgnoreString(sContribution);
...
void CToDoListWnd::DoPreferences(int nInitPage) 
{
	...
	if (nRet == IDOK)
	{
		...
		UpdateDefaultTaskAttributes(newPrefs);
		...
		// time tracking
		if (oldPrefs.GetTrackNonActiveTasklists() != newPrefs.GetTrackNonActiveTasklists())
...
BOOL CToDoListWnd::UpdateLanguageTranslationAndCheckForRestart(const CPreferencesDlg& oldPrefs)
{
	...
	BOOL bAdd2Dict = prefs.GetEnableAdd2Dictionary();
	...
	// else does the user want to enable/disable 'Add2Dictionary'
	if (bAdd2Dict != oldPrefs.GetEnableAdd2Dictionary())
	{
...
BOOL CToDoListWnd::LoadMenubar()
{
	...
	// delete 'Record Bug Report' if below W7 and 
	// delete 'Report Bug/Feature' if W7 or above
	if (COSVersion() < OSV_WIN7)
		m_menubar.GetSubMenu(AM_HELP)->DeleteMenu(ID_HELP_RECORDBUGREPORT, MF_BYCOMMAND);
	else
		m_menubar.GetSubMenu(AM_HELP)->DeleteMenu(ID_HELP_CONTACTUS, MF_BYCOMMAND);
	...
	if (startup.HasFlag(TLD_TASKLINK))
	{
		...
		if (startup.GetFilePaths(aFiles))
		{
			...
			if (sPath.IsEmpty() || ValidateTaskLinkFilePath(sPath))
				return DoTaskLink(sPath, dwTaskID);
		}
...
BOOL CToDoListWnd::DoPasteFromClipboard(CFilteredToDoCtrl& tdc, TDC_INSERTWHERE nWhere)
{
	...
	CString sImportPath = FileMisc::GetTempFileName(_T("ToDoList.import"), _T("txt"));
...
BOOL CToDoListWnd::ReloadTaskList(int nIndex, BOOL bNotifyDueTasks, BOOL bNotifyError)
{
	...
	TDC_FILE nRes = OpenTaskList(&tdc, tdc.GetFilePath());
...
void CToDoListWnd::OnFileSaveToUserStorage(UINT nCmdID) 
{
	...
	CString sTempPath = FileMisc::GetTempFileName();
	CFilteredToDoCtrl& tdc = GetToDoCtrl();
	CTaskFile tasks;

	if (!bUsesStorage && (tdc.Save(tasks) == TDCO_SUCCESS))
	{
		// copy to temp location
		FileMisc::CopyFile(tdc.GetFilePath(), sTempPath, TRUE, TRUE);
	}
	else if (tdc.Save(tasks, sTempPath) != TDCO_SUCCESS)
	{
		// TODO
		return;
	}
...
CFilteredToDoCtrl* CToDoListWnd::NewToDoCtrl(BOOL bVisible, BOOL bEnabled)
{
	...
	pTDC = new CFilteredToDoCtrl(m_mgrUIExtensions, m_mgrContent, 
								Prefs().GetDefaultCommentsFormat(), vis);
...
void CToDoListWnd::OnTabCtrlSelchanging(NMHDR* /*pNMHDR*/, LRESULT* pResult) 
{
	...
	if (m_nLastSelItem != -1)
	{
		...
		if (Prefs().GetAutoSaveOnSwitchTasklist() && !tdc.GetFilePath().IsEmpty() && tdc.IsModified())
		{
			...
			if (DoSaveWithBackupAndProgress(tdc, m_nLastSelItem, dummy) == TDCO_SUCCESS)
			{
				// update status
...
void CToDoListWnd::OnTabCtrlSelchange(NMHDR* /*pNMHDR*/, LRESULT* pResult) 
{
	...
	// update menu icon mgr for changes in source control status
...
void CToDoListWnd::UpdateToDoCtrlPreferences(CFilteredToDoCtrl* pTDC)
{
	...
	tdc.SetDefaultCategoryNames(aItems, userPrefs.GetDefaultListIsReadonly(PTDP_CATEGORY));

	if (userPrefs.GetDefaultListItems(PTDP_ALLOCTO, aItems))
		aItems.Append(m_tdiDefault.aAllocTo);
	
	tdc.SetDefaultAllocToNames(aItems, userPrefs.GetDefaultListIsReadonly(PTDP_ALLOCTO));
	
	userPrefs.GetDefaultListItems(PTDP_STATUS, aItems);
	tdc.SetDefaultStatusNames(aItems, userPrefs.GetDefaultListIsReadonly(PTDP_STATUS));
	
	userPrefs.GetDefaultListItems(PTDP_ALLOCBY, aItems);
	tdc.SetDefaultAllocByNames(aItems, userPrefs.GetDefaultListIsReadonly(PTDP_ALLOCBY));
...
BOOL CToDoListWnd::DoExit(BOOL bRestart, BOOL bClosingWindows) 
{
	...
	if (GetTDCCount() == 0)
	{
		...
		if (bRestart)
		{
			...
			if (FileMisc::Run(NULL, FileMisc::GetModuleFileName(), sParams) <= 32)
			{
				// TODO
			}
		}
...
void CToDoListWnd::OnToolsCheckout() 
{
	...
	if (m_mgrToDoCtrls.CheckOut(nSel, sCheckedOutTo))
	{
		// update UI
		UpdateCaption();
		UpdateMenuIconMgrSourceControlStatus();
	...
	else // failed -> notify user
	{
		...
		else
		{
			...
			sMessage.Format(IDS_CHECKOUTFAILED, sFilePath);
		}
...
void CToDoListWnd::OnToolsCheckin() 
{
	...
	if ((nSave == TDCO_SUCCESS) && m_mgrToDoCtrls.CheckIn(nSel))	
	{
		// Update UI
		UpdateCaption();
		UpdateMenuIconMgrSourceControlStatus();
...
void CToDoListWnd::OnUpdateWindow(CCmdUI* pCmdUI) 
{
	if (pCmdUI->m_pMenu)
	{
		...
		for (nTDC = 0; nTDC < MAXWINDOWS; nTDC++)
			pCmdUI->m_pMenu->DeleteMenu(ID_WINDOW1 + nTDC, MF_BYCOMMAND);
		...
		for (nTDC = 0; nTDC < nTDCCount; nTDC++)
		{
			CFilteredToDoCtrl& tdc = GetToDoCtrl(nTDC);
...
#ifdef _DEBUG		
		CString sMsg;
		sMsg.Format(_T("CToDoListWnd::DoTaskLink(%d, %s)"), dwTaskID, sPath);
		AfxMessageBox(sMsg);
#endif
...
LRESULT CToDoListWnd::OnTodoCtrlFailedLink(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	else // see if it's a task link
	{
		...
		CFilteredToDoCtrl::ParseTaskLink(szLink, dwTaskID, sPath);
