...
void TODOITEM::ResetCalcs(TDC_ATTRIBUTE nAttrib, BOOL bIncCustom) const 
{
	...
	case TDCA_POSITION:		
		SetAttribNeedsRecalc(TDIR_POSITION | TDIR_PATH);		
		break;
...
BOOL TODOITEM::RemoveLocalDependency(DWORD dwDependID)
{
	...
	{
		int nFind = FindDependency(dwDependID, nDepend);
...
{
	return (FindDependency(dwDependID) != -1);
}

int TODOITEM::FindDependency(DWORD dwDependID, int nSearchFrom) const
{
