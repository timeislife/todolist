...
CTDLImportExportComboBox::CTDLImportExportComboBox(const CImportExportMgr& mgrImpExp, BOOL bImport, BOOL bFileBasedOnly)
	: m_mgrImpExp(mgrImpExp), m_bImporting(bImport), m_bFileBasedOnly(bFileBasedOnly)
{
	m_nMinDLUHeight = 10;
}
								...
								DWORD dwItemData, const CString& sItem, BOOL bList)
{
	...
	if (nImage != -1)
	{
		...
		m_ilImages.GetImageList()->Draw(&dc, nImage, rImage.TopLeft(), ILD_TRANSPARENT);
	}
